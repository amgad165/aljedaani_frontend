...
const AdminNews: React.FC = () => {
  ...
  const fetchNews = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/news?active=all`, {
        headers: {
  ...
  const openEdit = (newsItem: NewsItem) => {
    const title       = getBilingual(newsItem.title);
    const description = getBilingual(newsItem.description);
    const blockquote  = getBilingual(newsItem.blockquote);
    const body        = getBilingual(newsItem.body);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingNews) {
        ...
        res = await fetch(`${API_BASE_URL}/news/${editingNews.id}`, {
          method: 'POST',
          headers: { 'Accept': 'application/json', ...authHeader },
          body: fd,
        });
      } else {
        res = await fetch(`${API_BASE_URL}/news`, {
          method: 'POST',
          headers: { 'Accept': 'application/json', ...authHeader },
          body: fd,
        });
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      const data = await res.json();
      if (data.success) { setDeleteConfirmId(null); fetchNews(); }
    ...
    <AdminLayout>
      <div style={{ padding: '32px 40px', fontFamily: 'Nunito' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28 }}>
          <div>
            <h1 style={{ margin: 0, fontSize: 28, fontWeight: 800, color: '#0a4d68' }}>News</h1>
            <div style={{ width: 56, height: 4, background: 'linear-gradient(90deg,#0a4d68,#05bfdb)', borderRadius: 4, marginTop: 6 }} />
          </div>
          <button style={btnPrimary} onClick={openCreate}>+ New News</button>
        </div>
        ...
        <div style={card}>
          {loading ? (
            <div style={{ textAlign: 'center', padding: 48, color: '#888' }}>Loading news…</div>
          ) : newsItems.length === 0 ? (
            <div style={{ textAlign: 'center', padding: 48, color: '#aaa' }}>No news yet. Click "New News" to add one.</div>
          ) : (
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
                <thead>
                  <tr style={{ borderBottom: '2px solid #e2e8f0' }}>
                    {['Image', 'Title', 'Author', 'Published', 'Updated', 'Read Time', 'Order', 'Status', 'Actions'].map(h => (
            ...
            <div style={{ direction: langTab === 'ar' ? 'rtl' : 'ltr' }}>
              <div style={{ marginBottom: 16 }}>
                ...
                <input
                  ...
                  placeholder={langTab === 'en' ? 'News title' : 'عنوان الخبر'}
                />
              </div>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <textarea
                  ...
                  placeholder={langTab === 'en' ? 'Brief excerpt shown on news cards' : 'ملخص يظهر على بطاقة الخبر'}
                />
              </div>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <textarea
                  ...
                  placeholder={langTab === 'en' ? 'Featured quote displayed at the top of the news item' : 'الاقتباس المميز في بداية الخبر'}
                />
              </div>

              <div style={{ marginBottom: 24 }}>
                <label style={labelStyle}>
                  News Body {langTab === 'ar' && '(محتوى الخبر)'}
                </label>
                <RichTextEditor
                  value={langTab === 'en' ? form.body_en : form.body_ar}
                  onChange={html => setForm(f => ({ ...f, [`body_${langTab}`]: html }))}
                  dir={langTab === 'ar' ? 'rtl' : 'ltr'}
                  placeholder={langTab === 'en' ? 'Start writing the news body…' : 'ابدأ كتابة محتوى الخبر…'}
                />
              </div>
            </div>
      ...
      {deleteConfirmId !== null && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: '#fff', borderRadius: 16, padding: '32px 36px', maxWidth: 400, width: '90%', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            ...
            <h3 style={{ margin: '0 0 8px', fontSize: 20, color: '#111' }}>Delete News?</h3>
