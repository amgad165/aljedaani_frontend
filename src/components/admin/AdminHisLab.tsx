...
import { getHisLabReports, getHisLabStats } from '../../services/hisLabService';
import type { HisLabReport, HisLabStats } from '../../services/hisLabService';

const AdminHisLab: React.FC = () => {
  const [reports, setReports] = useState<HisLabReport[]>([]);
  const [stats, setStats] = useState<HisLabStats | null>(null);
  ...
  const [selectedReport, setSelectedReport] = useState<HisLabReport | null>(null);
  ...
  const fetchData = async () => {
    try {
      const statsData = await getHisLabStats();
      setStats(statsData);
    } catch (error) {
      console.error('Error fetching HIS lab stats:', error);
    }
  };
  ...
  const fetchReports = async () => {
    setLoading(true);
    try {
      const data = await getHisLabReports(currentPage, perPage, {
      ...
      console.error('Error fetching HIS lab reports:', error);
      alert('Failed to load HIS lab reports');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRowClick = (report: HisLabReport) => {
      ...
      <div style={{ padding: '32px' }}>
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{ color: '#6b7280', fontSize: '16px' }}>
              Synced lab reports from Hospital Information System
            </p>
          </div>
        </div>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: '16px', alignItems: 'center', flexWrap: 'wrap' }}>
            <input
              type="text"
              placeholder="Search by File Number, SLNO, Lab Ref No, Patient Code..."
              value={searchTerm}
                ...
                <table style={{
                  ...
                  <thead>
                    <tr style={{ background: '#f9fafb' }}>
                      ...
                      <th style={tableHeaderStyle}>Lab Tech</th>
